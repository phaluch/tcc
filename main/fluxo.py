...
for videoPath in VIDEOS_TO_RUN:
    for TAMANHO_AREA_BUSCA in range(7,13, 3):
        for TAMANHO_BLOCO_ESTIMACAO in range(3,9, 3):
            for NUMERO_P_FRAMES in range(4,10, 2):
                for FATOR_COMPRESSAO in range(10,100, 10):
                    ...
                    formatedTargetOut = videoNameOut + f' FC({FATOR_COMPRESSAO}) PF({NUMERO_P_FRAMES}) SA({TAMANHO_AREA_BUSCA}) TBE({TAMANHO_BLOCO_ESTIMACAO})'
                    ...
                    cv2.imwrite(f'{videoOutput}/outputs/{formatedTargetOut}/{4*"0"}i.png',i_frame)
                    ...
                    if GENERATE_FINAL_VIDEO:
                        ...
                        os.system(f'ffmpeg -framerate 30 -i {videoOutput}/outputs/{formatedTargetOut}/%04d.jpeg -codec copy {videoOutput}/outputs/{formatedTargetOut}/output.mkv')
